...
| `BETA_E2E_SESSION_TOKEN_CRM`     | Optional CRM framework session token override, used to refresh that isolated host without replacing the fleet token map. Store it as a GitHub Actions secret.  |
| `BETA_E2E_SESSION_TOKEN_CHAT`    | Optional Chat framework session token override, used to refresh that isolated host without replacing the fleet token map. Store it as a GitHub Actions secret. |
...
| `agent.mainChatMaxOutputTokens`               | `AGENT_MAIN_CHAT_MAX_OUTPUT_TOKENS`                                                                                      | number  | `64000`                                      | Completion-token ceiling for the first attempt of an interactive chat turn.                                                                                                                                                                               |
| `agent.emptyResponseRetryMaxOutputTokens`     | `AGENT_EMPTY_RESPONSE_RETRY_MAX_OUTPUT_TOKENS`                                                                           | number  | `128000`                                     | Completion-token ceiling used when retrying a turn that came back with an empty final response. Must be at or above mainChatMaxOutputTokens to raise anything.                                                                                            |
...
| `app.homePath`                                | —                                                                                                                        | string  | —                                            | Private app route used after authentication. Apps default to /home; set this to / to keep the app at the root.                                                                                                                                            |
...
| `observability.mcpEvents`                     | `MCP_ANALYTICS`                                                                                                          | boolean | `true`                                       | Emit `$mcp_*` analytics events for the MCP server the app exposes.                                                                                                                                                                                        |
| `observability.mcpCaptureParameters`          | `MCP_ANALYTICS_PARAMETERS`                                                                                               | boolean | `false`                                      | Include redacted MCP tool-call arguments as $mcp_parameters. Off by default: arguments carry user content.                                                                                                                                                |
| `observability.mcpDebugInitialize`            | `MCP_DEBUG_INIT`                                                                                                         | boolean | `false`                                      | Log the clientInfo and capabilities of every MCP initialize handshake. Off by default: a handshake can carry client-specific metadata.                                                                                                                    |
...
| `runtime.agentChatStreaming`                  | `AGENT_NATIVE_AGENT_CHAT_STREAM_RUNTIME`                                                                                 | boolean | `false`                                      | Run the dedicated Nitro agent-chat response-streaming route used by an AWS Lambda Function URL.                                                                                                                                                           |
| `runtime.databaseUrlUnpooled`                 | `NETLIFY_DATABASE_URL_UNPOOLED`, `DATABASE_URL_UNPOOLED`                                                                 | string  | —                                            | Direct database URL for request-time clients when a serverless connection pooler is unavailable.                                                                                                                                                          |
